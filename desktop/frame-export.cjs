...
function fitVideo(width,height,fitMode,background='#000000'){
  if(fitMode==='cover')return`scale=${width}:${height}:force_original_aspect_ratio=increase:flags=lanczos,crop=${width}:${height}:(iw-${width})/2:(ih-${height})/2`;
  if(fitMode==='contain')return`scale=${width}:${height}:force_original_aspect_ratio=decrease:flags=lanczos,pad=${width}:${height}:(ow-iw)/2:(oh-ih)/2:${background}`;
...
function createFrameExport({ffmpeg,filePath,format='mp4',settings={},audioTracks=[],videoPassthrough=null,videoPlan=null,videoEncoder=null,cleanupFiles=[]}){
  ...
  const planned=!videoPassthrough&&videoPlan?.segments?.length?videoPlan:null,compositeSegments=planned?.segments.filter(segment=>segment.composite)||[],acceptsFrames=format!=='mp3'&&!videoPassthrough&&(!planned||compositeSegments.length>0),temporary=`${filePath}.${randomUUID()}.partial`;
  ...
  const copyAudio=Boolean(videoPassthrough?.copy&&videoPassthrough.audioCopy&&audioTracks.length===1&&audioTracks[0].path===videoPassthrough.path&&Math.abs(audioTracks[0].start)<.001&&Math.abs(audioTracks[0].end-config.duration)<.001&&Math.abs(audioTracks[0].sourceIn)<.001&&audioTracks[0].sourceOut>=config.duration-.02&&audioTracks[0].speed===1&&!audioTracks[0].reverse&&audioTracks[0].volume===1&&audioTracks[0].pan===0&&audioTracks[0].audioChannel==='stereo'&&!audioTracks[0].fadeIn&&!audioTracks[0].fadeOut);
  const inputs=copyAudio?[]:audioTracks.flatMap(track=>['-i',track.path]);
  const videoInputCount=planned?plannedSources.length+overlaySources.length+(compositeSegments.length?1:0):acceptsFrames||videoPassthrough?1:0,mix=copyAudio?'':audioMix(audioTracks,{inputOffset:videoInputCount});
  ...
  if(!['png','gif'].includes(format)){
    ...
    if(mix)args.push('-map','[audio]');else if(copyAudio)args.push('-map','0:a:0');
  }
  ...
  else if(videoPassthrough)args.push(...(videoPassthrough.copy?['-c:v','copy']:(videoEncoder?.args?.(config.quality)||['-c:v','libx264','-preset','veryfast','-threads','0','-crf',String(config.quality),'-pix_fmt','yuv420p'])),...(copyAudio?['-c:a','copy']:mix?['-c:a','aac','-b:a',config.audioBitrate]:['-an']),'-movflags','+faststart','-f',format);
