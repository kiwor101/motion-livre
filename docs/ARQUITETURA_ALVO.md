...
Proxies são gerados somente para vídeos acima de 1920×1080 (incluindo 2K e 4K), com controle visível para alternar entre proxy e original. O indicador permanece visível e desativado em vídeos até 1080p, mostrando que o original está em uso; proxies nunca são fonte da exportação final. Travamentos nessa faixa exigem investigação de decodificação, sincronização, cópias, reconstrução de interface e composição, sem mascará-los com proxy. O caso com mídia real e a correção do relógio estão registrados em `HANDOFF_BUG_PREVIEW_CORTES_AUDIO.md`.
