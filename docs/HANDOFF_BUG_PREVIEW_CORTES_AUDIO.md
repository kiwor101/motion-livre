...
## Trabalho da tarde — 2026-09-11

Foram ajustadas interações da timeline: miniaturas sem faixas pretas entre imagens, trim individual no cursor para a seleção atual, trim global somente quando todas as camadas estão selecionadas e deslocamento horizontal conjunto de seleções múltiplas. Regressões automatizadas confirmam que o trim mantém a borda oposta fixa, uma camada não selecionada não é alterada e clipes selecionados preservam suas distâncias durante o arraste.

O freeze passou a aguardar o quadro efetivamente apresentado pelo decoder e a capturar sempre o arquivo original, nunca o proxy reduzido. Novos freezes guardam caminho e instante de origem; freezes anteriores podem ser reconstruídos do vídeo vizinho durante a exportação quando a continuidade puder ser comprovada.

A exportação evoluiu de uma decisão binária entre cópia instantânea e compositor completo para um plano interno único por segmentos. Cortes sequenciais, velocidade e freezes compatíveis seguem diretamente pelo FFmpeg; segmentos do mesmo arquivo compartilham uma entrada/decoder, e o total final de quadros é limitado pela timeline completa. Resolução, FPS e CRF configurados são preservados, o redimensionamento usa Lanczos e o compositor continua responsável pelos recursos visuais ainda não representados pelo plano.

Passaram compilações TypeScript, suíte principal, suíte Electron, encoder FFmpeg real e uma regressão com vídeo em 2×, freeze e retomada do playback. **As melhorias desta tarde ainda não estão 100% validadas:** falta repetir no projeto e nos arquivos reais do usuário, comparar tempo de exportação antes/depois em resolução real, conferir visualmente qualidade, ruído, emendas, freezes e sincronização de áudio. Se houver regressão, não ampliar exceções por tipo de edição; corrigir o plano adaptativo único ou retornar com segurança ao compositor comprovado.
