...
export interface VideoRenderSegment {path:string;sourceStart:number;sourceDuration:number;duration:number;speed:number;freeze:boolean;fitMode?:'contain'|'cover'|'fill';background?:string;color?:string;composite?:boolean;overlayLayerIds?:number[];overlayBytes?:Uint8Array;timelineStart:number;startFrame:number;endFrame:number}
...
const defaults=(layer:Layer):boolean=>
  layer.x===50&&layer.y===50&&layer.depth===0&&layer.scale===100&&(layer.scaleX??layer.scale)===100&&(layer.scaleY??layer.scale)===100&&layer.rotation===0&&layer.opacity===100&&
...
export function ffmpegVideoPlan(project:ProjectState,format:ExportFormat,settings:ExportSettings):VideoRenderPlan|null {
  ...
  const base=(layer:Layer,_from:number,_to:number,startFrame:number,endFrame:number):VideoRenderSegment|null=>{const duration=(endFrame-startFrame)/settings.fps,timelineStart=settings.start+startFrame/settings.fps,common={duration,timelineStart,startFrame,endFrame},fitMode=['contain','cover','fill'].includes(layer.fitMode||'')?layer.fitMode as 'contain'|'cover'|'fill':'contain',fit=matchesFrame(layer,settings)?{}:{fitMode,background};if(!visualDefaults(layer))return null;if(layer.type==='video'&&layer.sourcePath){const speed=Math.max(.0625,Math.min(16,layer.speed||1)),sourceStart=sourceTimeForLayer(layer,timelineStart,layer.mediaDuration);return{path:layer.sourcePath,sourceStart,sourceDuration:duration*speed,speed,freeze:false,...fit,...common}}if(layer.type==='image'&&layer.frozenFrame){const index=visible.indexOf(layer),previous=visible[index-1],next=visible[index+1],path=layer.frozenSourcePath||previous?.sourcePath||next?.sourcePath,sourceStart=Number.isFinite(layer.frozenSourceTime)?Number(layer.frozenSourceTime):previous?.type==='video'&&close(previous.end,layer.start)?sourceTimeForLayer(previous,previous.end,previous.mediaDuration):next?.type==='video'&&close(next.start,layer.end)?sourceTimeForLayer(next,next.start,next.mediaDuration):NaN;return path&&Number.isFinite(sourceStart)?{path,sourceStart,sourceDuration:1/settings.fps,speed:1,freeze:true,...fit,...common}:null}return null};
