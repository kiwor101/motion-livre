...
export function create({document,media,viewport,canvas=document.createElement('canvas')}:{document:Document;media:RasterMediaSource;viewport:()=>DOMRect;canvas?:HTMLCanvasElement}):CompositionEngine {
  ...
  function* layers(project:ProjectState,time:number,width:number,height:number):Generator<HTMLCanvasElement> {
    if(scratch.width!==width)scratch.width=width;if(scratch.height!==height)scratch.height=height;
    const scratchContext=scratch.getContext('2d');if(!scratchContext)throw new Error('Canvas de rasterização indisponível');
    const visible=project.layers.filter(layer=>!['audio','null','camera'].includes(layer.type)&&layer.visible!==false&&time>=layer.start&&time<layer.end);
    // Blend modes need the accumulated backdrop, not an isolated transparent layer.
    // Until blend shaders land, reuse the same bounded rasterizer in preview/export.
    if(visible.some(layer=>layer.blend&&layer.blend!=='normal'&&layer.blend!=='source-over')){
      rasterizer.draw(scratchContext,scratch,time,project,alpha,visible);yield scratch;return;
    }
    for(const layer of visible){rasterizer.draw(scratchContext,scratch,time,project,true,[layer]);yield scratch}
  }
