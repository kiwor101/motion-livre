...
import {frameTime} from '../core/frame-sampling';
import {blendFrameSample,temporalSampleTimes} from '../core/temporal-sampling';
...
export function create({document,bridge,viewport,onProgress=()=>{},dependencies={}}:{document:Document;bridge:ExportBridge;viewport:()=>DOMRect;onProgress?:(progress:ExportProgress)=>void;dependencies?:Partial<ExportDependencies>}):ExportController {
  ...
  async function run(format:ExportFormat,project:ProjectState,settings:ExportSettingsInput={}):Promise<string|null> {
    ...
    try{
      ...
      const requested=services.normalize(format,settings,snapshot.composition,snapshot.duration),audioTracks=services.exportAudioTracks(snapshot as ProjectState,requested.start,requested.end),directVideo=passthroughVideo(snapshot,format,requested),videoPlan=ffmpegVideoPlan(snapshot,format,requested);
      ...
      progress('start',0);
      if(session.acceptsFrames){compositor=services.createComposition({document,media,viewport:()=>bounds});const frames=[new Uint8Array(config.width*config.height*4),new Uint8Array(config.width*config.height*4)],sampleFrame=new Uint8Array(config.width*config.height*4);let pendingWrite:Promise<void>|null=null;for(let position=0;position<frameIndices.length;position++){const index=frameIndices[position];token.abort.signal.throwIfAborted();const time=frameTime(index,config.fps,config.start,config.end),prepareAt=performance.now();await media.prepare(snapshot.layers,time,token.abort.signal);timings.prepareMs+=performance.now()-prepareAt;const bytes=frames[position%frames.length],sampleTimes=temporalSampleTimes(snapshot,time,config.fps,config.start,config.end,config.transparent);for(let sampleIndex=0;sampleIndex<sampleTimes.length;sampleIndex++){const composeAt=performance.now();if(!compositor.render(snapshot,sampleTimes[sampleIndex],config.width,config.height,config.transparent))throw new Error('Contexto WebGL indisponível durante exportação');timings.composeMs+=performance.now()-composeAt;const readAt=performance.now(),sample=compositor.readFrame(sampleIndex===0?bytes:sampleFrame);timings.readbackMs+=performance.now()-readAt;if(sampleIndex>0)blendFrameSample(bytes,sample,sampleIndex)}if(pendingWrite)await pendingWrite;const transferAt=performance.now();pendingWrite=bridge.writeExportFrame(bytes).then(()=>{timings.transferMs+=performance.now()-transferAt});progress('rendering',position+1)}if(pendingWrite)await pendingWrite}
