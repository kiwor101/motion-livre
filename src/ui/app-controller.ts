...
export function createAppController():AppControllerContext {
  ...
  context.switchPanel=name=>{window.dispatchEvent(new CustomEvent('motion:panel-change',{detail:name}));document.querySelectorAll<HTMLElement>('.panel').forEach(element=>element.classList.toggle('active',element.id===`panel-${name}`))};window.addEventListener('motion:select-panel',(event:Event)=>context.switchPanel((event as CustomEvent<string>).detail));
