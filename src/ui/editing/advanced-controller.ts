...
document.addEventListener('keydown',e=>{const editing=Boolean((e.target as Element|null)?.closest('input,textarea,select,[contenteditable="true"]'));if(!editing&&e.ctrlKey&&e.key.toLowerCase()==='z'){e.preventDefault();$('#undoBtn').click()}if(!editing&&e.ctrlKey&&e.key.toLowerCase()==='y'){e.preventDefault();$('#redoBtn').click()}if(!editing&&e.key==='Delete'&&state.selection.selected)$('#deleteLayer').click();if(!editing&&e.code==='Space'){e.preventDefault();$('#playBtn').click()}});
