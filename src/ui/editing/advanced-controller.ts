...
function syncComposition(){const c=state.composition,fps=$<HTMLSelectElement>('#compFps'),fpsValue=String(Number(c.fps.toFixed(3))),range=state.renderRange||{start:0,end:state.duration},slider=$<HTMLInputElement>('#timeSlider');if(![...fps.options].some(option=>option.value===fpsValue))fps.add(new Option(fpsValue,fpsValue));$('#compWidth').value=String(c.width);$('#compHeight').value=String(c.height);fps.value=fpsValue;$('#compDuration').value=String(state.duration);$('#compBackground').value=c.background;$('#stage').style.background=c.background;slider.min=String(Math.max(0,range.start));slider.max=String(Math.min(state.duration,range.end))}context.syncComposition=syncComposition;
