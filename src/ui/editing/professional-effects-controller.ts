...
export function installProfessionalEffectsController(context:ProfessionalEffectsContext):ProfessionalEffectsController {
  ...
  const renderEffectStack=()=>{const layer=context.selected();if(!layer?.id){window.dispatchEvent(new CustomEvent('motion:effect-stack-update',{detail:{items:[],hasSelection:false}}));return}ensureDefaults(layer);const active=layer.effectOrder.filter(key=>labels[key]&&isActive(layer,key));for(const key of Object.keys(labels))if(!layer.effectOrder.includes(key)&&isActive(layer,key))active.push(key);const items=active.map(key=>({key,label:`${labels[key]}${key==='rgb'?` ${layer.effects.redGain}/${layer.effects.greenGain}/${layer.effects.blueGain}`:`: ${layer.effects[key]}`}`}));window.dispatchEvent(new CustomEvent('motion:effect-stack-update',{detail:{items,hasSelection:true}}))};
  window.addEventListener('motion:effect-stack-action',event=>{const detail=(event as CustomEvent<{action:'up'|'down'|'remove';key:string}>).detail,layer=context.selected();if(!layer?.id)return;const refresh=()=>{setProperties(context.state,{id:layer.id!,values:{filter:buildFilter(layer)}});context.updateSelected();renderEffectStack();context.markDirty()};if(detail.action==='remove'){if(resetEffect(context.state,{id:layer.id,key:detail.key})){refresh();context.syncProps()}}else if(moveEffect(context.state,{id:layer.id,key:detail.key,direction:detail.action==='up'?-1:1}))refresh()});
