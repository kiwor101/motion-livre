...
export function installVectorEditingController(context:VectorEditingContext):void {
  ...
  const addTransformHandles=()=>{removeTransformHandles();const layer=context.selected();if(!layer?.id||layer.locked||layer.type==='audio')return;const element=layerElement(layer.id);if(!element)return;
    const start=(event:PointerEvent,kind:'scale'|'rotate')=>{event.preventDefault();event.stopPropagation();const bounds=byId('stage').getBoundingClientRect(),centerX=bounds.left+bounds.width*layer.x/100,centerY=bounds.top+bounds.height*layer.y/100,startScaleX=layer.scaleX??layer.scale,startScaleY=layer.scaleY??layer.scale,startRotation=layer.rotation,startKeyframes=structuredClone(layer.keyframes),startDistance=Math.hypot(event.clientX-centerX,event.clientY-centerY)||1,startAngle=Math.atan2(event.clientY-centerY,event.clientX-centerX);context.history.begin(context.snapshot());
      const move=(pointer:PointerEvent)=>{const ratio=Math.hypot(pointer.clientX-centerX,pointer.clientY-centerY)/startDistance,values=kind==='scale'?{scaleX:Math.max(5,Math.min(800,startScaleX*ratio)),scaleY:Math.max(5,Math.min(800,startScaleY*ratio))}:{rotation:startRotation+(Math.atan2(pointer.clientY-centerY,pointer.clientX-centerX)-startAngle)*180/Math.PI};setAnimatedProperties(context.state,{id:layer.id!,time:context.state.playback.time,values});context.updateSelected();context.syncProps();context.renderTimeline()};
