...
export function installVectorEditingController(context:VectorEditingContext):void {
  ...
  const addTransformHandles=()=>{removeTransformHandles();const layer=context.selected();if(!layer?.id||layer.locked||layer.type==='audio')return;const element=layerElement(layer.id);if(!element)return;
    const start=(event:PointerEvent,kind:'scale'|'rotate')=>{event.preventDefault();event.stopPropagation();const bounds=byId('stage').getBoundingClientRect(),centerX=bounds.left+bounds.width*layer.x/100,centerY=bounds.top+bounds.height*layer.y/100,startScale=layer.scale,startRotation=layer.rotation,startKeyframes=structuredClone(layer.keyframes),startDistance=Math.hypot(event.clientX-centerX,event.clientY-centerY)||1,startAngle=Math.atan2(event.clientY-centerY,event.clientX-centerX);context.history.begin(context.snapshot());
      const move=(pointer:PointerEvent)=>{const values=kind==='scale'?{scale:Math.max(5,Math.min(800,startScale*Math.hypot(pointer.clientX-centerX,pointer.clientY-centerY)/startDistance))}:{rotation:startRotation+(Math.atan2(pointer.clientY-centerY,pointer.clientX-centerX)-startAngle)*180/Math.PI};setAnimatedProperties(context.state,{id:layer.id!,time:context.state.playback.time,values});context.updateSelected();context.syncProps();context.renderTimeline()};
      const up=(pointer:PointerEvent)=>{removeEventListener('pointermove',move);removeEventListener('pointerup',up);removeEventListener('pointercancel',up);if(pointer.type==='pointercancel'){setProperties(context.state,{id:layer.id!,values:{scale:startScale,rotation:startRotation}});layer.keyframes=startKeyframes;context.history.cancel();context.updateSelected();context.syncProps();context.renderTimeline()}else context.history.commit(context.snapshot());context.markDirty()};
