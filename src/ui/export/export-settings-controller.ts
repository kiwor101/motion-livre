...
export function installExportSettingsController(context:ExportSettingsContext):void {
  ...
  document.querySelectorAll<HTMLElement>('[data-export-format]').forEach(button=>{
    button.onclick=()=>{
      ...
      const fps=byId<HTMLSelectElement>('exportFps'),hasAnimatedTransform=state.layers.some(layer=>layer.keyframes.length>1),recommendedFps=hasAnimatedTransform&&state.composition.fps<50?60:state.composition.fps,fpsValue=String(Number(recommendedFps.toFixed(3)));if(![...fps.options].some(option=>option.value===fpsValue))fps.add(new Option(fpsValue,fpsValue));fps.value=fpsValue;
