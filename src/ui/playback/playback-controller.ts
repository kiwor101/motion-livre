...
export function installPlaybackController(context:PlaybackControllerContext):PlaybackController {
  ...
  const tick=(now:number)=>{if(!context.state.playback.playing)return;const range=bounds(),time=(now-context.state.playback.started)/1000;if(time>=range.end){setTime(range.end);stop()}else{setTime(time);animationFrame=requestAnimationFrame(tick)}};
