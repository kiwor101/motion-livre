...
export function installPlaybackController(context:PlaybackControllerContext):PlaybackController {
  ...
  const bounds=()=>{const start=Math.max(0,Math.min(context.state.duration,context.state.renderRange?.start||0)),candidate=context.state.renderRange?.end??context.state.duration,end=Math.max(start,Math.min(context.state.duration,candidate));return{start,end:end>start?end:context.state.duration}};
  const setTime=(time:number)=>{const range=bounds();context.state.playback.time=Math.max(range.start,Math.min(range.end,time));slider.min=String(range.start);slider.max=String(range.end);slider.value=String(context.state.playback.time);byId<HTMLElement>('timeLabel').textContent=`${formatTime(context.state.playback.time)} / ${formatTime(range.end)}`;context.mediaRuntime.sync(context.state);for(const listener of listeners)listener();document.dispatchEvent(new Event('motion:scenechange'))};
  ...
  const tick=(now:number)=>{if(!context.state.playback.playing)return;const range=bounds(),mediaTime=context.mediaRuntime.playbackTime(context.state),time=mediaTime===null?context.state.playback.time:mediaTime===undefined?(now-context.state.playback.started)/1000:mediaTime;if(mediaTime!==undefined&&mediaTime!==null)context.state.playback.started=now-time*1000;if(time>=range.end){setTime(range.end);stop()}else{setTime(time);animationFrame=requestAnimationFrame(tick)}};
  const play=()=>{const range=bounds();if(context.state.playback.time<range.start||context.state.playback.time>=range.end)setTime(range.start);context.mediaRuntime.start();context.state.playback.playing=true;context.state.playback.started=performance.now()-context.state.playback.time*1000;playButton.textContent='❚❚';setTime(context.state.playback.time);animationFrame=requestAnimationFrame(tick)};
  playButton.onclick=()=>context.state.playback.playing?stop():play();byId<HTMLButtonElement>('toStart').onclick=()=>{stop();setTime(bounds().start)};slider.oninput=()=>{stop();setTime(Number(slider.value))};
