...
export function createTimelineClipGestureController(context:TimelineClipGestureContext):(event:PointerEvent,layer:Layer)=>void {
  return (event,layer)=>{
    ...
    let destination:TrackId|null=trackIdFor(layer),valid=true,last=event,animation=0,newTrack:NewTrackTarget|null=null,ripple:RippleInsertionPlan|null=null;const guide=document.createElement('div');guide.className='drop-guide';context.timeline.append(guide);const dropFeedback=createTimelineDropFeedback(context.timeline,context.headerWidth);
    let moved=false;
    const update=(pointer:PointerEvent)=>{last=pointer;if(!moved&&Math.hypot(pointer.clientX-startX,pointer.clientY-event.clientY)<3)return;moved=true;const delta=(pointer.clientX-startX+context.timeline.scrollLeft-startScroll)/context.width()*context.state.duration;let value=(mode==='right'?original.end:original.start)+delta;value=Math.round(value/context.frameDuration())*context.frameDuration();
      ...
      guide.style.left=`${context.headerWidth+layer.start/context.state.duration*context.width()}px`;
      guide.textContent=`${timeText(layer.start)} → ${timeText(layer.end)}${groupMove?` · ${group.length} clipes`:ripple?` · inserir e abrir ${timeText(ripple.shift)}`:newTrack?` · nova camada ${newTrack.before?'acima':'abaixo'}`:''}${valid?'':' · ocupado'}`;
    ...
    const end=(pointer:PointerEvent)=>{cancelAnimationFrame(animation);removeEventListener('pointermove',update);removeEventListener('pointerup',end);removeEventListener('pointercancel',end);guide.remove();dropFeedback.remove();for(const item of group)context.timeline.querySelector<HTMLElement>(`[data-clip="${item.id}"]`)?.classList.remove('dragging');if(!moved){context.history.cancel();return}if(!valid||pointer.type==='pointercancel'){for(const item of group)Object.assign(item,originals.get(item.id!)!);context.history.cancel();context.renderLayers();context.syncProps();context.setTime(context.state.playback.time);return}
