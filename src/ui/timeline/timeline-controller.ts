...
import {getTimelineToolbar} from './timeline-toolbar';
...
export function installTimelineController(context:TimelineControllerContext):void {
  ...
  let position=()=>{};
  const {groups,attachTrackControls,renameTrack,renderLayersPanel}=createTimelineTracksController({state:context.state,pushHistory:context.pushHistory,commit,selectLayer:context.selectLayer});
  const {tools,beatButton,rangeStatus}=getTimelineToolbar(document.querySelector<HTMLElement>('.timeline-controls')!);
  ...
  const renderer=createTimelineRenderer({state:context.state,timeline,head,rangeStatus,headerWidth:header,pixelsPerSecond,groups,trackControls:attachTrackControls,renameTrack,beginMarquee,edit:(event,layer)=>edit(event,layer),preview:context.timelineMedia.preview,sourceTimeForLayer:context.sourceTimeForLayer,showContextMenu,dragMarker,dragRenderBoundary,navigation,selectLayer:context.selectLayer,setTime,pushHistory:context.pushHistory,commit,renderAudioMixer:context.renderAudioMixer,renderLayersPanel});
  renderTimeline=renderer.renderTimeline;context.replaceRenderTimeline(renderTimeline);
  edit=createTimelineClipGestureController({state:context.state,timeline,headerWidth:header,width,frameDuration,history:context.history,snapshot:context.snapshot,stop:context.stop,selectClip,selectLayer:context.selectLayer,geometry:renderer.geometry,preview:context.timelineMedia.preview,syncProps:context.syncProps,setTime:value=>setTime(value),renderLayers:context.renderLayers,syncComposition:context.syncComposition,commit});
  ...
  tools.addEventListener('click',event=>{const action=event.target instanceof Element?event.target.closest<HTMLElement>('[data-action]')?.dataset.action:undefined;if(action==='range-in')setRenderBoundary('start');if(action==='range-out')setRenderBoundary('end');if(action==='trim-start')trimTimeline('start');if(action==='trim-end')trimTimeline('end')});
