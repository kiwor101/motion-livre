...
export function installTimelineNavigationController(context:TimelineNavigationContext):TimelineNavigationController {
  ...
  const position=()=>{const duration=Math.max(context.state.duration,.001),left=context.headerWidth+context.state.playback.time/duration*context.width();context.head.style.left=`${left}px`;context.head.style.visibility=left<context.headerWidth+context.timeline.scrollLeft?'hidden':'visible';const output=document.getElementById('timelinePosition');if(output)output.textContent=`${timeText(context.state.playback.time)} / ${timeText(context.state.duration)}`;for(const [action,key] of [['reverse','reverse'],['flip','flipX']] as const)context.tools.querySelector(`[data-action="${action}"]`)?.setAttribute('aria-pressed',String(Boolean(context.selected()?.[key])))};
