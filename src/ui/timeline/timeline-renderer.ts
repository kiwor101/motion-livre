...
export function createTimelineRenderer(context:TimelineRendererContext):TimelineRenderer {
  ...
  const dragKeyframe=(event:PointerEvent,layer:Layer,visibleIndex:number)=>{
    ...
    context.selectLayer(layer.id);context.setTime(target.time);context.pushHistory();const startTime=target.time,originalTimes=new Map(layer.keyframes.map(frame=>[frame,frame.time])),dot=event.currentTarget as HTMLElement;dot.classList.add('dragging-keyframe');let moved=false;
    const startX=event.clientX;const move=(pointer:PointerEvent)=>{if(!moved&&Math.abs(pointer.clientX-startX)<=3)return;const frame=1/(context.state.composition?.fps||30),bounds=context.timeline.getBoundingClientRect(),pointerTime=(pointer.clientX-bounds.left+context.timeline.scrollLeft-context.headerWidth)/context.pixelsPerSecond(),time=Math.round(pointerTime/frame)*frame;moved=Math.abs(time-startTime)>.0001;moveKeyframe(context.state,{id:layer.id!,index:layer.keyframes.indexOf(target),time});context.setTime(Math.max(layer.start,Math.min(layer.end,time)));const clip=context.timeline.querySelector<HTMLElement>(`[data-clip="${layer.id}"]`);if(clip){geometry(clip,layer);const liveIndex=visibleKeyframes(layer).indexOf(target);clip.querySelectorAll<HTMLElement>('.key-dot')[liveIndex]?.classList.add('dragging-keyframe')}};
    const end=(pointer:PointerEvent)=>{removeEventListener('pointermove',move);removeEventListener('pointerup',end);removeEventListener('pointercancel',end);context.timeline.querySelectorAll('.dragging-keyframe').forEach(marker=>marker.classList.remove('dragging-keyframe'));if(pointer.type==='pointercancel'){for(const [frame,time] of originalTimes)frame.time=time;layer.keyframes.sort((left,right)=>left.time-right.time);renderTimeline();return}if(moved)context.commit();else renderTimeline()};
