...
(() => {
  ...
  const actions={play:['play','Reproduzir / pausar'],split:['scissors','Dividir no cursor'],duplicate:['copy','Duplicar em camada acima'],delete:['trash-2','Excluir clipe'],freeze:['snowflake','Congelar quadro'],reverse:['rotate-ccw','Reproduzir ao contrário'],flip:['flip-horizontal-2','Espelhar horizontalmente'],up:['arrow-up','Mover para cima'],down:['arrow-down','Mover para baixo']};
  ...
  $('#muteBtn').onclick=()=>{state.playback.previewMuted=!state.playback.previewMuted;$('#muteBtn').textContent=state.playback.previewMuted?'🔇':'🔊';$$('#stage video,#stage audio').forEach(v=>{const l=state.layers.find(l=>l.id===+v.parentElement.dataset.id);v.muted=state.playback.previewMuted||!!l?.muted});setTime(state.playback.time)};
  ...
  function align(){const r=$('#stage').getBoundingClientRect(),ratio=options.value==='stage'?r.width/r.height:options.value.split('/').reduce((a,b)=>Number(a)/Number(b)),w=Math.min(r.width,r.height*ratio),h=w/ratio;guides.style.width=w+'px';guides.style.height=h+'px';guides.querySelector('span').textContent=options.selectedOptions[0].text}
  ...
  const prev=setTime;setTime=function(t){prev(t);refreshTransport()};
