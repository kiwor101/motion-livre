...
require('./electron-test-runtime.cjs').isolateUserData(app,'alight-xml');
...
app.whenReady().then(async()=>{
  ...
  try{
    ...
    await window.webContents.executeJavaScript(`(async()=>{
      ...
      const {state,addLayer,loadProjectData}=motionEditor,motionMedia=motionEditor.mediaRuntime,motionPreview=motionEditor.preview;
      ...
      addLayer('rect','','Antes da importação');await Promise.resolve();motionEditor.pushHistory();const before=JSON.stringify(motionEditor.projectData());
      try{alightCompat.importScene('<scene><shape></scene>',{silent:true})}catch{}check(JSON.stringify(motionEditor.projectData())===before,'Malformed XML mutated project');
      ...
      const saved=motionEditor.projectData();loadProjectData(saved);check(state.layers[0].alightEffects[0].properties[0].keyframes[0].time===-0.021459&&state.alightScene.attributes.amver==='1002351','Native save/reopen lost Alight metadata');
