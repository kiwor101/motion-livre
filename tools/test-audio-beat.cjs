...
(async()=>{
 ...
 try{
  ...
  await page.evaluate(()=>{
   ...
   state.selection.selected=video.id;setTime(5);renderLayers();selectLayer(video.id);
  });
  ...
  await page.evaluate(()=>{state.markers=[1.5];state.selection.selected=state.layers.find(l=>l.type==='audio').id;renderLayers();selectLayer(state.selection.selected)});
