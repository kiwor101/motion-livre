...
(async()=>{
  ...
  let writePending=false,overlapped=false,writes=0;const pipelinedController=create({document:{},bridge:{...bridge,beginFrameExport:async()=>({started:true,acceptsFrames:true,settings:{width:1,height:1,frameCount:3,fps:1,start:0,end:3}}),writeExportFrame:async()=>{writes++;writePending=true;await new Promise(resolve=>setTimeout(resolve,15));writePending=false}},viewport:()=>({}),dependencies:{...dependencies,createMedia:()=>({prepare:async()=>{if(writePending)overlapped=true},destroy:async()=>{},size:0})}});assert.equal(await pipelinedController.run('mp4',{layers:[],duration:3,composition:{width:1,height:1,fps:1,background:'#000000'}}),'/result.mp4');assert.equal(writes,3);assert.equal(overlapped,true,'Frame preparation did not overlap encoder transfer');
