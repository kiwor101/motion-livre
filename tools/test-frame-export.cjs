...
(async()=>{
  ...
  try{
    ...
    session=createFrameExport({ffmpeg:path.join(dir,'absent.exe'),filePath:path.join(dir,'failed.mp4'),settings:{width:64,height:64,end:1}});await new Promise(resolve=>setTimeout(resolve,50));await assert.rejects(session.write(Buffer.alloc(64*64*4)));await session.cancel();assert.deepEqual(await fs.readdir(dir),['direct.mp4','movie.mp4','trimmed.mp4']);
