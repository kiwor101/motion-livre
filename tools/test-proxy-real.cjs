...
(async()=>{const dir=await fs.mkdtemp(path.join(os.tmpdir(),'motion-proxy-real-')),ffmpeg=path.resolve('vendor/ffmpeg/ffmpeg.exe'),probe=path.resolve('vendor/ffmpeg/ffprobe.exe');try{
 const input=path.join(dir,'source.mp4');await run(ffmpeg,['-y','-f','lavfi','-i','testsrc2=size=2560x1440:rate=24:duration=0.3','-c:v','libx264','-preset','ultrafast',input],{windowsHide:true});
 ...
 const results=await Promise.all(Array.from({length:4},()=>cache(input,{width:2560,height:1440})));assert.equal(calls,1);assert.ok(results.every(r=>r.path===results[0].path));assert.equal((await cache(input,{width:2560,height:1440})).cached,true);
 ...
 const failed=Proxy.createCache({directory:path.join(dir,'failed'),generate:async(i,o)=>{await fs.writeFile(o,'partial');throw Error('expected')}});await assert.rejects(failed(input,{width:2560,height:1440}),/expected/);assert.deepEqual(await fs.readdir(path.join(dir,'failed')),[]);
 console.log('PASS: real 1440p proxy, concurrent deduplication, cache and failure cleanup');
