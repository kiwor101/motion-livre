...
assert.deepEqual(ffmpegVideoPlan(project([clip(0,5,0,{mediaWidth:1280,fitMode:'cover'})]),'mp4',settings).segments.map(segment=>[segment.composite,segment.fitMode,segment.startFrame,segment.endFrame]),[[undefined,'cover',0,150]]);
const squareSettings=normalize('mp4',{width:1080,height:1080},project([]).composition,5),squarePlan=ffmpegVideoPlan(project([clip(0,5,0,{fitMode:'cover'})]),'mp4',squareSettings);
assert.deepEqual(squarePlan.segments.map(segment=>[segment.composite,segment.fitMode,segment.background]),[[undefined,'cover','#000000']]);
assert.deepEqual(ffmpegVideoPlan(project([clip(0,5,0,{fitMode:'cover',x:51})]),'mp4',squareSettings).segments.map(segment=>segment.composite),[true]);
