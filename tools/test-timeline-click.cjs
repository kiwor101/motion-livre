...
app.whenReady().then(async()=>{
  ...
  try{
    ...
    await window.webContents.executeJavaScript(`(async()=>{
      ...
      const currentClip=document.querySelector('[data-clip="'+layer.id+'"]'),oldTile=currentClip.querySelector('img[data-slot="1"]'),oldLeft=oldTile?.getBoundingClientRect().left;
      if(oldLeft===undefined)throw Error('Miniatura fixa não foi criada');
      layer.start=.017;motionEditor.renderLayers();
      const newTile=document.querySelector('[data-clip="'+layer.id+'"] img[data-slot="1"]');
      if(!newTile||Math.abs(newTile.getBoundingClientRect().left-oldLeft)>1)throw Error('Miniatura existente se moveu ao estender a borda: '+oldLeft+' → '+newTile?.getBoundingClientRect().left);
      if(!document.querySelector('[data-clip="'+layer.id+'"] img[data-slot="0"]'))throw Error('Trecho revelado não recebeu miniatura');
    })()`);
