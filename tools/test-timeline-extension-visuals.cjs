...
app.whenReady().then(async()=>{
  ...
  try{
    ...
    await window.webContents.executeJavaScript(`(async()=>{
      ...
      const newAudioLabel=clip(audio.id).querySelector('.clip-label'),newAudioClipLeft=clip(audio.id).getBoundingClientRect().left;if(Math.abs(newAudioLabel.getBoundingClientRect().left-newAudioClipLeft-4)>1||Math.abs(newAudioLabel.getBoundingClientRect().width-oldAudioLabel.width)>1)throw Error('O título do áudio não acompanhou a nova borda inicial');
    })()`);
