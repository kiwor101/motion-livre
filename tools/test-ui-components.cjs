...
app.whenReady().then(async () => {
  ...
  try {
    ...
    await window.webContents.executeJavaScript(`(async () => {
      ...
      check(document.querySelectorAll('.library > .tools > [data-panel]').length === 6, 'Asset tool button contracts changed')
      ...
      check(document.querySelectorAll('.property-content input[type="range"]').length >= 16, 'Effect property field components changed')
      ...
      check(!document.getElementById('unsafeEffectMarkup'), 'Dynamic panel data was interpreted as HTML')
      ...
      check(document.getElementById('propX').type === 'number' && document.getElementById('propX').getBoundingClientRect().width > 40, 'OpenCut-style transform number field changed')
      ...
      check(document.querySelector('.time-ruler > .ruler-corner')?.textContent === '', 'TimelineRuler corner should stay visually empty')
      ...
      check(markerStyle.top === '2px' && markerStyle.width === '11px' && markerStyle.pointerEvents === 'auto', 'TimelineMarker scoped geometry changed')
      const inlineKeyframe = document.querySelector('[data-keyframe-property="x"]')
      check(inlineKeyframe && inlineKeyframe.getAttribute('aria-pressed') === 'false', 'Inline transform keyframe control was not rendered')
      ...
      document.querySelector('.time-ruler').dispatchEvent(new PointerEvent('pointerdown', { bubbles: true, button: 0, clientX: bounds.left + 108 + 6.1 * pixelsPerSecond }))
      dispatchEvent(new PointerEvent('pointerup'))
      check(Math.abs(motionEditor.state.playback.time - 6) < .001, 'Playhead did not snap to the beat marker')
      document.querySelector('.time-ruler').dispatchEvent(new PointerEvent('pointerdown', { bubbles: true, button: 0, clientX: bounds.left + 108 + 6.35 * pixelsPerSecond }))
      dispatchEvent(new PointerEvent('pointerup'))
